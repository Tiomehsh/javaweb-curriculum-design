...
document.addEventListener('DOMContentLoaded', function() {
    ...
    fetch('../api/department/list')
        ...
        .then(data => {
            if (data.success) {
                const deptSelect = document.getElementById('visitDept');
                ...
                console.error('获取部门列表失败:', data.message);
            }
        })
        .catch(error => {
...
function generateSummary() {
    ...
    const officialTitle = document.getElementById('officialTitle').value;
    const visitDept = document.getElementById('visitDept');
    const visitDeptText = visitDept.options[visitDept.selectedIndex].text;
    const visitContact = document.getElementById('visitContact').value;
    ...
    summaryContent.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; font-size: 0.9em;">
            <div><strong>联系人：</strong>${name}</div>
            <div><strong>联系电话：</strong>${phone}</div>
            <div><strong>单位名称：</strong>${unitName}</div>
            <div><strong>职务：</strong>${officialTitle}</div>
            <div><strong>访问部门：</strong>${visitDeptText}</div>
            <div><strong>接待人：</strong>${visitContact || '未指定'}</div>
            <div><strong>校区：</strong>${campus}</div>
            <div><strong>访问时间：</strong>${formattedTime}</div>
            <div><strong>访问人数：</strong>${visitors}人</div>
            <div><strong>交通方式：</strong>${transportation}</div>
            <div style="grid-column: 1 / -1;"><strong>来访目的：</strong>${purpose}</div>
        </div>
    `;
}
...
function submitAppointment() {
    ...
    const appointmentData = {
        ...
        officialTitle: document.getElementById('officialTitle').value,
        visitDeptId: document.getElementById('visitDept').value,
        visitContact: document.getElementById('visitContact').value,
    ...
    // 发送预约请求
    fetch('../api/appointment/official/add', {
        method: 'POST',
        headers: {
