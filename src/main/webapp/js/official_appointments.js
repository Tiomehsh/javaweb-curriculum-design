...
function loadDepartments() {
    ...
    // 检查是否是部门管理员
    const isDepartmentAdmin = document.getElementById('is-department-admin').value === 'true';
...
// 加载预约列表
function loadAppointments(searchTerm = '', statusFilter = '', departmentFilter = '', dateFilter = '') {
    ...
    // 构建查询参数
    let url = '../api/appointment/official/list';
    let queryParams = [];
    
    // 如果是部门管理员，只查看自己部门的预约
    if (isDepartmentAdmin) {
        url = `../api/appointment/official/dept/${departmentId}`;
    } else if (departmentFilter) {
        url = `../api/appointment/official/dept/${departmentFilter}`;
    }
        ...
        .then(data => {
            ...
            if (!isDepartmentAdmin) {
                ...
                if (departmentFilter) {
                    Array.from(departmentFilter.options).forEach(option => {
                        if (option.value) {
                            departmentNames[option.value] = option.text;
                        }
                    });
                }
            }
            ...
            data.forEach(appointment => {
                ...
                tr.innerHTML = `
                    ...
                    <td>${departmentNames[appointment.visitDeptId] || appointment.visitDeptName || appointment.visitDeptId || '-'}</td>
...
function viewAppointment(appointmentId) {
    // 发送请求获取预约详情
    fetch(`../api/appointment/official/${appointmentId}`)
        .then(response => response.json())
        .then(appointment => {
            ...
            detailsHtml += `
                ...
                <div class="detail-section">
                    ...
                    <table class="detail-table">
                        ...
                        <tr>
                            <th>职务</th>
                            <td colspan="3">${appointment.officialTitle || '-'}</td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="detail-section">
                    ...
                    <table class="detail-table">
                        <tr>
                            <th>访问部门</th>
                            <td>${appointment.visitDeptName || appointment.visitDeptId || '-'}</td>
                            <th>访问校区</th>
...
function submitApproval() {
    ...
    // 发送审批请求
    fetch(`../api/appointment/official/${isReject ? 'reject' : 'approve'}/${currentAppointmentId}`, {
        method: 'POST',
        headers: {
...
function confirmComplete() {
    ...
    // 发送完成预约请求
    fetch(`../api/appointment/official/complete/${currentAppointmentId}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
